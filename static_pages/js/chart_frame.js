...
function UpdateMCFromLB()
{
  ...
  fetch('/bitcoinaverage/ticker-all-currencies/')
    ...
    function UpdateMCInfo() {
      ...
      if (LUX != 'BTC')
      {
        $('#id_MC_info').append($('<li class="breadcrumb-item">1 ' + LUX + ' = '+BTC.toFixed(8)+' BTC</li>'));
      ...
      {
        $('#id_MC_info').append($('<li class="breadcrumb-item">1 ' + LUX + ' = '+USD.toFixed(2)+' USD</li>'));
...
function SetChartLegend()
{
  ...
  const LUX = coinNameToTicker[utils.MAIN_COIN].ticker; 
  const COIN = coinNameToTicker[g_CurrentPair].ticker
  

  $.getJSON( "/api/v1/public/getmarketsummary?market="+LUX+"-"+COIN+"&period="+g_currentChartPeriod, ret => {
    if (!ret || !ret.success || ret.success != true || LUX != coinNameToTicker[utils.MAIN_COIN].ticker || COIN != coinNameToTicker[g_CurrentPair].ticker) 
      return;
    ...
    const legend = $(
      '<ul class="nav" style="line-height: 30px;">'+
        ...
        '<li class="nav-item mr-3"><h4>'+COIN+' / '+LUX+'</h4></li>'+
