...
{
  const LUX = coinNameToTicker[utils.MAIN_COIN] ? coinNameToTicker[utils.MAIN_COIN].ticker || 'LUX' : 'LUX'; 
  const bodyModal = 
    '<table class="table">'+
      '<tr>'+
        '<th>Order</th>'+
      ...
      '<tr>'+
        ...
        '<td>'+order.coin+'</td>'+
        '<td>'+order.price.toFixed(8)*1+" "+LUX+'</td>'+
      '</tr>'
    '</table>';
...
function UpdateMarket(message)
{
 ...
 // lastupdated = Date.now();
  ...
  for (var i=0; i<message.coins.length; i++)
  {
    ...
    const prevPrice = g_LastPrices[LUX+'-'+BTC] || 0;
    g_LastPrices[LUX+'-'+BTC] = price;
    
    const prevVolume = g_LastVolumes[LUX+'-'+BTC] || 0;
    g_LastVolumes[LUX+'-'+BTC] = vol;
...
//    if (rowClass.length != 0) bNeedUpdate = true;
   // else if (rowClass == chColor) chColor = "";
    ...
    const tr = $('<tr class="'+rowClass+'"></tr>')
      ...
      .on('click', e => {
        ...
        utils.ChangeUrl(document.title + "(" + coinName + ' market)', '/market/'+LUX+'-'+BTC);
  ...
  utils.ChangeUrl(document.title + "(" + g_CurrentPair+' market)', '/market/'+LUX+'-'+BTC+(window.location.search || ""));
...
function UpdateBuySellTickers()
{
  ...
  $('#id_price_buy').text(LUX);
  $('#id_price_sell').text(LUX);
  
  $('#id_comission_buy').text(LUX);
  $('#id_comission_sell').text(LUX);
  
  $('#id_total_buy').text(LUX);
  $('#id_total_sell').text(LUX);
...
function UpdateHelpers()
{
  ...
  SetHelper("inputSellTotal", LUX, LB_data);
  SetHelper("inputSellComission", LUX, LB_data);
  SetHelper("inputSellPrice", LUX, LB_data);
  SetHelper("inputBuyTotal", LUX, LB_data);
  SetHelper("inputBuyComission", LUX, LB_data);
  SetHelper("inputBuyPrice", LUX, LB_data);
  SetHelper("buy_balance_button", LUX, LB_data);
  SetHelper("button_max_ask", LUX, LB_data);
  SetHelper("button_max_bid", LUX, LB_data);
  
  function SetHelper(name, LUX, LB_data)
  {
    ...
    if (LUX != 'BTC')
      $('#' + name).attr('title', utils.MakePrice2(total) + " LUX = " + utils.MakePrice2(LB_data.BTC*total) + " BTC" + helper); 
    else
      $('#' + name).attr('title', utils.MakePrice2(total) +" LUX" + helper); 
  }
