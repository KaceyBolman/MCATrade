...
function UpdateWallet(data)
{
    ...
    const LUX = coinNameToTicker[utils.MAIN_COIN] ? coinNameToTicker[utils.MAIN_COIN].ticker || 'BTC' : 'BTC';
    ...
    let tdCoin = '';
     if (data.coin.ticker == LUX) {
        tdCoin = $('<td scope="col" class="align-middle"> '+icon+unescape(data.coin.name)+'</td>');
    } else {
	    tdCoin = $('<td scope="col" class="align-middle"> <a href="/market/'+LUX+'-'+data.coin.ticker+'">'+icon+unescape(data.coin.name)+'</a></td>');
    }
...
//      message = '<div class="p-3 mb-2 bg-danger text-white">WARNING!!! ---TTC--- IS NOT TittieCoin !!!</div>';
  ...
  $.getJSON( "/api/v1/public/getmarketsummary?market="+utils.MAIN_COIN+"-"+coinTicker, ret => {
    ...
    const passwordGroup = $(
      '<div class="form-group">'+
        '<label for="id_password" class="control-label  requiredField">'+utils.MCATrade+' password<span class="asteriskField">*</span> </label>'+
